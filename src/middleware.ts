...
export default function middleware(request: NextRequest) {
  ...
  if (request.nextUrl.pathname === '/posts/categories') {
    return NextResponse.rewrite(new URL(`/posts/categories/dev/pages/1`, request.url));
  }
  ...
  if (match && match[1]) {
    ...
    return NextResponse.rewrite(new URL(`/posts/categories/${category}/pages/1`, request.url));
  }
